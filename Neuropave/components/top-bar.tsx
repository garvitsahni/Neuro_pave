  ...
  return (
    <header className="h-16 flex items-center justify-between px-6 border-b border-white/[0.04] bg-[#0b0b0b]/80 backdrop-blur-xl sticky top-0 z-40">
      ...
      <div className="flex items-center gap-4">
        <div>
          <p className="text-[10px] uppercase tracking-[0.2em] text-white/25 font-bold">Smart Bharat Platform</p>
      ...
      <div className="flex items-center gap-3">
        {/* Notifications */}
        <button className="relative p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.06] hover:border-white/[0.1] transition-smooth-fast group">
          <Bell className="w-4 h-4 text-white/40 group-hover:text-white/60 transition-smooth-fast" />
          {/* Notification dot */}
        ...
        <Link href="/predictions">
          <button className="relative group flex items-center gap-2 h-10 px-4 bg-gradient-to-r from-zinc-700 to-zinc-500 hover:from-zinc-600 hover:to-zinc-400 text-white text-xs font-bold rounded-xl transition-smooth shadow-lg shadow-zinc-500/20 active:scale-[0.98] ring-1 ring-white/10">
            <Sparkles className="w-3.5 h-3.5" />
            <span>Run Prediction</span>
            {/* Glow effect */}
            <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-zinc-300/10 to-zinc-100/10 opacity-0 group-hover:opacity-100 transition-smooth" />
          </button>
        </Link>
        ...
        {/* User */}
        <button className="flex items-center gap-2.5 p-1.5 pr-3 rounded-xl hover:bg-white/[0.04] transition-smooth-fast group">
          ...
          <ChevronDown className="w-3 h-3 text-white/20 group-hover:text-white/40 transition-smooth-fast hidden lg:block" />
        </button>
      </div>
    </header>
  );
